...
export default class WebAppView extends Component {
  ...
  attachHandlers = () => {
    ...
    this._webview.current.addEventListener(
      'ipc-message',
  ...
  detachHandlers = () => {
    this._webview.current.removeEventListener(
      'new-window',
  ...
  handleNavigate = event => {
    ...
    if (
      event.url.includes('.slack.com') &&
      (event.url.includes('/signin') || event.url.includes('/signout/done'))
    ) {
